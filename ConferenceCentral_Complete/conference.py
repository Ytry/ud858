...
SESSION_GET_TOPIC_REQUEST = endpoints.ResourceContainer(
    message_types.VoidMessage,
...
# - - - Conference objects - - - - - - - - - - - - - - - - -
    ...
    def _formatFilters(self, filters):
        ...
        for f in filters:
            ...
            if filtr["operator"] != "=":
                # check if inequality operation has
                # been used in previous filters
                # disallow the filter if inequality
                # was performed on a different field before
                # track the field on which the
...
# - - - Session objects - - - - - - - - - - - - - - - - - - -
    ...
    def createSessionObject(self, request):
        ...
        # convert dates and times from strings to
        # Date/Time objects respectively
        if data['date']:
...
# - - - Session queries - - - - - - - - - - - - - - - - - - -
    ...
    @endpoints.method(SESSION_GET_TOPIC_REQUEST, SessionForms,
                      path='getSessionsByTopic',
                      http_method='GET',
